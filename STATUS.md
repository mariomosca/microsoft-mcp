...
## v0.4.0 (27 Aug 2026) — 4 tool SharePoint read-only (brief Leo 27/08) — DEPLOYATA 27/08 12:44, revision `0000024`
...
- **Scope — CORREZIONE post-regressione (27/08 pomeriggio)**: la prima conclusione "nessun admin consent necessario" era SBAGLIATA — lo smoke test locale girava sull'app registration stdio (`61682051`, che ha `Files.Read.All`+`Sites.Read.All`), mentre il gateway usa l'app `4506b2a9` che aveva solo `Files.ReadWrite` (OneDrive personale) → tutti gli sp_* 403 in prod (segnalato da Leo, token OAuth fresco). Fix: delegated **`Files.Read.All`** aggiunto all'app gateway + admin consent (Mario, 27/08 13:49) — copre site drives e Search API driveItem; `Sites.Read.All` resta non necessario (solo per endpoint `/sites`). Nessun redeploy: OBO usa `.default`, scope attivo alla prossima emissione token (subito con re-login connettore, altrimenti ≤1h per cache OBO). Lezione: validare sempre sul path OBO di produzione, non solo stdio locale — le due app registration NON hanno gli stessi permessi.
...
- Deploy: ok Mario 27/08 ~10:13, eseguito 27/08 12:44 CEST. `az acr build` run cb13 (58s) + `az containerapp update` → revision `brandart-mcp-gateway--0000024` RunningAtMaxScale 100% traffico, `/health` 200, startup log pulito. Contatore leak fastmcp azzerato dal deploy. NB: Claude Desktop cachea la tool-list → ricaricare il connettore per vedere i 4 tool sp_*.
...
- **v0.4.0** (27 Aug 2026) — 4 tool SharePoint read-only: `sp_search` (cross-site driveItem search), `sp_list_folder` (children per item_id/path/root), `sp_get_file` (estrazione testo server-side o download via save_path), `sp_get_excel` (Graph Excel API: fogli/usedRange/cell_range come TSV, fallback openpyxl). Nessun nuovo scope Graph (drive-scoped endpoints coperti da Files.ReadWrite delegato; Sites.Read.All volutamente non richiesto). Revision `0000024`. Commit `03e7d6d`. Rollback: redeploy v0.3.5.
